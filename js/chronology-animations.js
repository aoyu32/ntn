        ...
        this.sections.forEach(section => {
            ...
            if (sectionId === 'chronology-cdz') {
                if (wrapperLeft) {
                    ...
                    // 文本从下方进入
                    if (leftText) gsap.set(leftText, { y: 300, opacity: 1 });
                    
                    // 图片从上方进入
                    if (leftImage) gsap.set(leftImage, { y: -300, opacity: 1 });
                }
            ...
            if (sectionId === 'chronology-hl') {
                if (wrapperRight) {
                    ...
                    // 设置图片的初始状态：在右边视口外，顺时针旋转90度
                    if (rightImage) {
                        gsap.set(rightImage, { 
                            opacity: 1,
                            rotation: 90,  // 顺时针旋转90度
                            transformOrigin: "right bottom", // 设置旋转原点为右下角
                            x: 500,  // 向右偏移，确保在视口外
                            y: 0     // 不需要垂直偏移
                        });
                    }
                    
                    // 设置文本的初始状态：在右边视口外，逆时针旋转-90度
                    if (rightText) {
                        gsap.set(rightText, { 
                            opacity: 1,
                            rotation: -90,  // 逆时针旋转90度
                            transformOrigin: "right top", // 设置旋转原点为右上角
                            x: 500,  // 向右偏移，确保在视口外
                            y: 0     // 不需要垂直偏移
                        });
                    }
